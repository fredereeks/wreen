    ...
    return (
        <form ref={formRef} action={handleClientSubmit} className="flex flex-col gap-2 py-10 max-w-sm md:max-w-md">
            <div className="relative flex flex-col gap-4 max-w-xl pb-5 text-center mx-auto">
                <h3 className="text-sky-500 text-xl sm:text-2xl font-semibold sm:font-bold">Let&apos;s go <span className="text-dark/80">S<span className="text-primary">cr</span>iPT</span> down your Idea?</h3>
            </div>
            ...
            <button type="submit" className="rounded-full text-sm mt-2 py-2 px-6 sm:px-10 w-max shadow-md shadow-red-500 bg-red-500 text-white cursor-pointer">{pending ? 'Processing...' : 'Login to Continue'}</button>
            <Link href={'/signup'} className="col-span-2 pt-2 text-center text-base text-slate-400">New to Fleen? <span className="underline">Signup</span></Link>
        </form>
    )
}
